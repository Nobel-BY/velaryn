...
> 当前版本：**v0.9.2 Beta** · Android 12+ · ARM64 · 横屏平板优先。Beta 阶段请为重要资料保留独立备份。
...
- **研究工作流**：提供 `/研究`、`/比较`、`/学习计划`、`/整理` 和 `/原型规划`，支持用 `@资料`、`@知识库` 明确限定上下文。
- **证据轨迹**：保留查询、来源、冲突与结论依据；证据不足时明确提示。
- **创作空间**：管理项目规则、任务、文件、代码、版本和隔离预览，智能体修改代码前必须展示补丁并获得确认。
- **安全恢复**：修改前自动创建 Checkpoint，批量写入失败时回滚。
- **迁移导入**：新增 Obsidian Vault 与 Notion 导出包的独立导入入口。
